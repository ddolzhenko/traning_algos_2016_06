...
void test_general(TFunc base_search_func)
{
    ...
    test(-1, search_func, Vec({54,23}), key );

    //usual
    test(-1, search_func, Vec({-3,2, 23,  54, 6}), key );
    test(-1, search_func, Vec({1,  2, -80, 5, 98}), key );
    test(-1, search_func, Vec({1,  2,  1, 90, 3}), key );
    ...
    //usual
    test(0, search_func, Vec({key,2, 23,  54, 6}), key );
    test(4, search_func, Vec({1,  2, -80, 5,  key}), key );
    test(2, search_func, Vec({1,  2, key, 90, 3}), key );
}
